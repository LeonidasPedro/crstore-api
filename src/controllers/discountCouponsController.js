...
const getById = async (req, res) => {
  try {
    ...
    let response = await DiscountCoupon.findOne({
      where: {
        id
      }
    });
    ...
    if (!response) {
      return res.status(400).send({
        message: `Não foi encontrado cupom com o id ${id}`
      });
    }
...
const update = async (id, data, res) => {
  ...
  if (!response) {
    return res.status(400).send({ type: 'error', message: `Não foi encontrado nenhum cupom com o id ${id}` })
  }
  ...
  await response.save();
  return res.status(200).send({
    message: `cupom ${id} atualizado com sucesso`,
    data: response
  });
}
...
const destroy = async (req, res) => {
  try {
    ...
    if (!id) {
      return res.status(200).send({
        type:'error',
        message: 'Informe um id válido para deletar o cupom'
      });
    }
    ...
    if (!response) {
      return res.status(200).send({ message: `Não foi encontrado cupom com o id ${id}` })
    }

    await response.destroy();
    return res.status(200).send({
      message: `cupom id ${id} deletado com sucesso`
    })
  } catch (error) {
    return res.status(200).send({
      type: 'error',
